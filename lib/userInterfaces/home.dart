...
// Extra padding for iPhone 15 Pro
          ...
          Positioned(
            left: (screenWidth * 0.8 - 2) *
                (pointsManager.pointsToPass /
                    (pointsManager.maxPoints == 0
                        ? 1
                        : pointsManager.maxPoints)),
            top: 0,
            bottom: 0,
            child: Container(
              width: 2,
  ...
  Widget _buildDraggableTimeBlock(BuildContext context, TaskModel task) {
    ...
    final DateTime topTime =
        (_draggingTaskId == task.id && _tentativeStart != null)
            ? _tentativeStart!
            : task.startTime;
    final double top =
        topTime.hour * hourHeight + topTime.minute * hourHeight / 60;
    ...
    return Positioned(
      ...
      child: GestureDetector(
        onVerticalDragUpdate: task.isMovable
            ? (details) {
                setState(() {
                  _tentativeStart = (_tentativeStart ?? task.startTime).add(
                      Duration(
        ...
        onVerticalDragEnd: task.isMovable
            ? (details) async {
                ...
                TaskModel updatedTask =
                    task.copyWith(startTime: _tentativeStart!);
                ...
                if (!hasConflict) {
                  ...
                  if (selectedTask == null) {
                    ...
                    scheduleManager.editTask(
                        overlappingTask.copyWith(endTime: _tentativeStart!));
                  }
                  _fetchSchedule();
                }
        ...
        child: GestureDetector(
                ...
                return Padding(
                  ...
                  child: TaskDetails(
                    ...
                    onComplete: (bool nowComplete) {
                      setState(() {
                        TaskModel updatedTask =
                            task.copyWith(isComplete: nowComplete);
                        scheduleManager.editTask(updatedTask);
                        nowComplete
          ...
          child: Opacity(
            opacity: (_draggingTaskId == task.id && _tentativeStart != null)
                ? 0.7
                : 1.0,
            child: Container(
